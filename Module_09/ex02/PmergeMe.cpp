...
{
	std::vector<std::pair<int,int>>	pair_int;
	std::vector<std::pair<int,int>>	pair;

	make_pair_int(vect, pair_int);
}*/


/*void	make_pair(std::vector<std::pair<int,int>> &pair)
{
	std::vector<std::pair<int,int>> pair_recurs;

	std::vector<std::pair<int,int>>::iterator	it = pair.begin();
	while (pair_recurs.size() != (pair.size() / 2))
	{
		pair_recurs.push_back(std::make_pair(std::get<0>(*it), std::get<0>(*(++it))));
	}
	if (it != pair.end())
		pair_recurs.push_back(std::make_pair(std::get<0>(*it), -1));
	if (pair_recurs.size() > 2)
		make_pair(pair_recurs);
	//make_insert();
}*/

/*void	make_pair_int(std::vector<long int> &vect, std::vector<std::pair<int,int>> &pair_int)
{
	for (std::vector<long int>::iterator i = vect.begin(); i < vect.end(); ++i)//recorre la lista de los numeros
	{
		if (i == vect.end())
			pair_int.push_back(std::make_pair(static_cast<int>(*i), -1));//si son numeros impares, lo indico con -1
		pair_int.push_back(std::make_pair(static_cast<int>(*i), static_cast<int>(*(++i))));//hace std::pair y lo mete en el vector
	}
}*/
